...
model_cfgs = dict(  # cfg配置  372-684除必要全注释掉
    # Fiddling with configs / defaults / still pretraining
    ...
    #     **_rw_max_cfg(rel_pos_type='mlp'),
    # ),
    maxvit_rmlp_small_rw_224=MaxxVitCfg(  # cfg配置地区  todo
        embed_dim=(96, 192, 384, 768),
        depths=(2, 2, 5, 2),
        block_type=('M',) * 4,
        stem_width=(32, 64),
    ...
    # maxvit_rmlp_small_rw_256=MaxxVitCfg(  # cfg配置地区
    #     embed_dim=(96, 192, 384, 768),
    #     depths=(2, 2, 5, 2),
    #     block_type=('M',) * 4,
    #     stem_width=(32, 64),
...
class MbConvBlock(nn.Module):  # todo
    ...
    def __init__(
        ...
        # ----------------------zmx
        self.convadd1 = create_conv2d(in_chs, in_chs, kernel_size=1, stride=1)  # zmx add
        # ----------------------zmx
...
class MaxxVit(nn.Module):  # todo
    ...
    def __init__(
        ...
        transformer_cfg = cfg_window_size(cfg.transformer_cfg, img_size)  # zmx 为啥这里size没有
        ...
        #----------------------zmx
        # self.convadd = create_conv2d(64,96, kernel_size=1, stride=2)  # zmx add
        #----------------------zmx

        stride = self.stem.stride
        feat_size = tuple([i // s for i, s in zip(img_size, to_2tuple(stride))])  # feature_size(112.112)

        num_stages = len(cfg.embed_dim)  # 重要一步
        assert len(cfg.depths) == num_stages
        dpr = [x.tolist() for x in torch.linspace(0, drop_path_rate, sum(cfg.depths)).split(cfg.depths)]  # 创建一个2252的list|通过这个过程，原始的线性间隔张量被分割成了多个部分，并将每个部分转换为Python列表。结果存储在列表dpr中。每个列表元素代表了不同的部分，对应于cfg.depths中的元素。这个过程通常用于在深度学习模型中实现DropPath操作，其中drop_path_rate是控制丢弃路径的概率
        in_chs = self.stem.out_chs
        #final_norm_layer = get_norm_layer(cfg.transformer_cfg.norm_layer)
        stages = []
        #norms = []
        for i in range(num_stages):  # 这里拼装maxvitstage?
            stage_stride = 2
            out_chs = cfg.embed_dim[i]
            feat_size = tuple([(r - 1) // stage_stride + 1 for r in feat_size])  # （56，56）
            stages += [MaxxVitStage(  # 这里这里
                in_chs,
                out_chs,
                depth=cfg.depths[i],
