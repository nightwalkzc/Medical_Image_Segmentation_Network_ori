...
from segmentation_mask_overlay import overlay_masks  # segmentation_mask_overlay改过
    ...
    if len(image.shape) == 3:
        ...
        for ind in range(image.shape[0]):
            ...
            with torch.no_grad():  # 禁用梯度计算
                ...
                #print(len(P))
                outputs = 0.0
                for idx in range(len(P)):  #每个类别预测结果累加
                    outputs += P[idx]
                out = torch.argmax(torch.softmax(outputs, dim=1), dim=1).squeeze(0)
                out = out.cpu().detach().numpy()
                if x != patch_size[0] or y != patch_size[1]: # # 如果输入图像尺寸与 patch 尺寸不匹配，进行插值缩放
                ...
                # # fig_gt = overlay_masks(image[ind, :, :], masks, labels=mask_labels, colors=cmap, alpha=0.5)  # zmx add
                # # fig_pred = overlay_masks(image[ind, :, :], preds_o, labels=mask_labels, colors=cmap, alpha=0.5)  # zmx add
