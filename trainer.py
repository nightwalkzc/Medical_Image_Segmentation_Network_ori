...
def trainer_synapse(args, model, snapshot_path):
    ...
    l = [0, 1, 2, 3]
    ss = [x for x in powerset(l)]  # 生成给定序列 seq 的幂集
    #ss = [[0],[1],[2],[3]]
    print(ss)
    for epoch_num in iterator:
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            for s in ss:
                ...
                for idx in range(len(s)):
                    iout += P[s[idx]]  # 将子集 s 中的每个元素对应的 P 中的值累加到 iout 上
            ...
            lr_ = base_lr
            for param_group in optimizer.param_groups:  # 更新优化器中所有参数组的学习率
                param_group['lr'] = lr_
        ...
        if epoch_num < start_epoch:
            performance = inference(args, model, best_performance)  # zmx add
        ...
        if epoch_num >= val_start_epoch and epoch_num % val_epoch == 0 and epoch_num<eva_epo :  # zmxadd
...
#  ===================调参部分代码
def train_continue(args, model, snapshot_path):
    logging.basicConfig(filename=snapshot_path + "/log.txt", level=logging.INFO,
                        format='[%(asctime)s.%(msecs)03d] %(message)s', datefmt='%H:%M:%S')
    logging.getLogger().addHandler(logging.StreamHandler(sys.stdout))
    logging.info(str(args))
    ...
    for epoch_num in iterator:
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            # 训练前做一次验证
            P = model(image_batch)  # P = MERIT_cascaded
            ...
            for s in ss:
                ...
                if (s == []):
                    continue
                for idx in range(len(s)):
                    iout += P[s[idx]]  # todo 没看懂 | 将列表 'P' 中对应子集 's[idx]' 的概率值累加到 'iout' 上
                    # zmx add
                    # iout += P[s[idx]] * idx
                    # zmx add
                loss_ce = ce_loss(iout, label_batch[:].long())
                loss_dice = dice_loss(iout, label_batch, softmax=True)  # 论文中的softmax在这里使用
                loss += (lc1 * loss_ce + lc2 * loss_dice)  # 把ss16个列表的loss全加在一起
